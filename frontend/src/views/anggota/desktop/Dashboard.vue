...
const closeCreateStory = () => { showCreateStoryModal.value = false }
...
<template>
  <div class="min-h-screen bg-slate-950 text-slate-100 flex font-sans select-none antialiased">
            ...
            <button 
              ...
              :class="['w-full py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all cursor-pointer', 
                (currentUser.following || []).includes(viewingMember.email) 
                  ? 'bg-slate-800 text-slate-300 hover:bg-rose-50/10 hover:text-rose-400 hover:border-rose-500/35 border border-slate-750' 
    ...
    <transition name="toast-anim">
      <div v-if="showToast" class="fixed bottom-8 right-8 z-50 px-6 py-4 bg-emerald-600 border border-emerald-500/20 text-white rounded-2xl shadow-2xl flex items-center justify-between gap-4 font-black text-xs uppercase tracking-widest">
