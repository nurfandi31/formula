...
<template>
  <div class="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans select-none antialiased">
    <!-- Mobile Header -->
    <header class="h-14 bg-slate-950 border-b border-slate-900 px-6 flex items-center justify-between sticky top-0 z-30 shadow-md">
      <h2 class="logo-cursive text-white select-none">Formula</h2>
      <div class="flex items-center gap-5">
        <button @click="openCreateStory" class="text-2xl text-white hover:text-emerald-400 transition-colors active:scale-90 select-none">＋</button>
        <button @click="openNotifications" class="relative text-xl active:scale-90 select-none">
          ❤️
    ...
    <!-- Mobile Main Content -->
    <main class="flex-grow pb-24 px-4 py-6 relative overflow-hidden bg-slate-950">
      <div class="absolute top-0 right-0 w-72 h-72 bg-emerald-500/5 blur-[90px] rounded-full -z-10"></div>
      <div class="absolute bottom-0 left-1/4 w-72 h-72 bg-teal-500/5 blur-[90px] rounded-full -z-10"></div>
    ...
    <!-- Edit Post Modal -->
    <div v-if="editingPost" class="fixed inset-0 bg-slate-950/85 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
      <div class="bg-slate-900 border border-slate-800 rounded-lg w-full max-w-sm overflow-hidden relative shadow-2xl animate-in zoom-in-95 duration-300">
        <div class="p-5 border-b border-slate-850 flex justify-between items-center bg-slate-900/60">
          <h3 class="text-xs font-black uppercase tracking-wider text-white">Edit Kiriman</h3>
        ...
        <div class="p-5 space-y-5">
          ...
          <div class="space-y-1">
            <label class="text-[8px] font-black text-slate-400 uppercase tracking-widest">Caption</label>
            <textarea v-model="editingCaptionText" rows="3" class="w-full px-3 py-2.5 bg-slate-950 border border-slate-850 rounded-xl text-xs font-bold text-slate-200 focus:outline-hidden focus:ring-1 focus:ring-emerald-500"></textarea>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <button @click="confirmDeletePost(editingPost)" class="py-3 bg-rose-600/10 border border-rose-500/20 hover:bg-rose-600 text-rose-450 hover:text-white rounded-xl font-black text-[10px] uppercase tracking-wider transition-all cursor-pointer">Hapus</button>
            <button @click="saveEditedPost" class="py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-black text-[10px] uppercase tracking-wider transition-all cursor-pointer">Simpan</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="viewingMember" class="fixed inset-0 bg-slate-950/85 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
      <div class="bg-slate-900 border border-slate-800 rounded-lg w-full max-w-sm overflow-hidden relative shadow-2xl animate-in zoom-in-95 duration-300">
        <div class="p-5 border-b border-slate-850 flex justify-between items-center bg-slate-900/60">
          <h3 class="text-xs font-black uppercase tracking-wider text-white">Profil Pengurus</h3>
        ...
        <div class="p-6 text-center space-y-5">
          <div class="w-20 h-20 rounded-full p-1 bg-linear-to-tr from-emerald-500 to-teal-500 mx-auto shadow-xl">
          ...
          <div class="space-y-0.5">
            <h4 class="text-base font-black text-white leading-none">{{ viewingMember.name || viewingMember.nickname }}</h4>
            <span class="text-[8px] font-black text-emerald-500 uppercase tracking-widest inline-block mt-1">{{ viewingMember.title || viewingMember.role || 'Anggota FORMULA' }}</span>
          </div>

          <div v-if="currentUser && viewingMember.email !== currentUser.email">
            <button 
              @click="handleFollowToggle(viewingMember.email)"
              :class="['w-full py-2.5 rounded-xl font-black text-[10px] uppercase tracking-wider transition-all cursor-pointer', 
                (currentUser.following || []).includes(viewingMember.email) 
                  ? 'bg-slate-800 text-slate-355 border border-slate-700' 
                  : 'bg-emerald-600 text-white hover:bg-emerald-700'
              ]"
            >
              {{ (currentUser.following || []).includes(viewingMember.email) ? 'Mengikuti' : 'Ikuti' }}
            </button>
          </div>

          <p class="text-xs text-slate-400 italic leading-relaxed">"{{ viewingMember.quote || 'Bersama FORMULA, berkontribusi aktif melahirkan kegunaan konkret bagi dusun tercinta.' }}"</p>
          <div class="grid grid-cols-2 gap-2 pt-2 font-bold text-[9px] uppercase tracking-wider text-slate-500 text-left border-t border-slate-850">
            <div class="space-y-1 bg-slate-950/50 p-2.5 rounded-lg border border-slate-850">
              <span class="text-[7px] font-black text-slate-600 block">🎓 PENDIDIKAN</span>
              <span class="text-slate-300">{{ viewingMember.education || 'S1 Pendidikan' }}</span>
            </div>
            <div class="space-y-1 bg-slate-950/50 p-2.5 rounded-lg border border-slate-850">
              <span class="text-[7px] font-black text-slate-600 block">🏆 HOBI</span>
    ...
    <!-- Notifications Modal -->
    <div v-if="showNotifications" class="fixed inset-0 bg-slate-950/85 backdrop-blur-xs flex items-start justify-center p-4 pt-16 z-50 animate-in fade-in duration-200">
      <div class="bg-slate-900 border border-slate-800 rounded-lg w-full max-w-sm overflow-hidden relative shadow-2xl animate-in slide-in-from-top-4 duration-300">
        <div class="p-4 border-b border-slate-850 flex justify-between items-center bg-slate-900/60">
          <h3 class="text-xs font-black uppercase tracking-wider text-white">Notifikasi</h3>
        ...
        <div class="p-2 max-h-[50vh] overflow-y-auto">
          <div v-if="!currentUser?.notifications || currentUser.notifications.length === 0" class="p-6 text-center text-slate-500 text-xs">
            Belum ada notifikasi baru.
          </div>
          <div v-else class="divide-y divide-slate-800/40">
            <div v-for="notif in currentUser.notifications" :key="notif.id" class="p-3.5 flex items-center gap-3.5">
              <div @click="viewMemberProfile(getNotificationUser(notif.fromEmail)); showNotifications = false" class="w-9 h-9 rounded-full border border-slate-700 overflow-hidden cursor-pointer flex-shrink-0">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-xs text-slate-300 leading-snug">
                  <span class="font-bold text-white" @click="viewMemberProfile(getNotificationUser(notif.fromEmail)); showNotifications = false">
                    {{ getNotificationUser(notif.fromEmail)?.name || 'Seseorang' }}
                  </span> 
                  mulai mengikuti Anda.
                </p>
                <p class="text-[8px] text-slate-500 mt-0.5 tracking-wider">{{ notif.time }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <transition name="toast-anim">
      <div v-if="showToast" class="fixed bottom-20 right-4 left-4 z-50 px-5 py-3.5 bg-emerald-600 border border-emerald-500/20 text-white rounded-xl shadow-2xl flex items-center justify-between gap-4 font-black text-[10px] uppercase tracking-wider">
        <div class="flex items-center gap-3">
          <span class="flex h-1.5 w-1.5 rounded-full bg-white animate-pulse"></span>
          <span>{{ toastMsg }}</span>
        </div>
        <button @click="showToast = false" class="text-white hover:text-emerald-250 transition-colors text-[9px] font-bold">✕</button>
      </div>
    </transition>
  </div>
</template>
