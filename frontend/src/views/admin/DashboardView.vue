...
// Generate reactive unpaid cash list dynamically from the main member users list
...
<template>
  <AdminLayout>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="bg-gradient-to-br from-indigo-500 to-blue-600 rounded-lg p-6 shadow-[0_10px_30px_-10px_rgba(79,70,229,0.5)] border border-indigo-400/30 relative overflow-hidden">
        <div class="absolute -top-6 -right-6 w-32 h-32 rounded-full bg-white opacity-10 blur-2xl pointer-events-none"></div>
        <div class="flex items-start justify-between relative z-10">
          <div>
            <h3 class="text-xs font-black uppercase tracking-widest mb-1" style="color: rgba(255,255,255,0.8) !important">Total Pengurus</h3>
            <p class="text-3xl font-black leading-none" style="color: #ffffff !important">{{ totalUsers }}</p>
          </div>
        ...
        <div class="mt-4 text-[10px] font-black uppercase tracking-wider relative z-10" style="color: rgba(255,255,255,0.9) !important">↑ Aktif & Terverifikasi</div>
      </div>
      
      <div class="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg p-6 shadow-[0_10px_30px_-10px_rgba(16,185,129,0.5)] border border-emerald-400/30 relative overflow-hidden">
        <div class="absolute -top-6 -right-6 w-32 h-32 rounded-full bg-white opacity-10 blur-2xl pointer-events-none"></div>
        <div class="flex items-start justify-between relative z-10">
          <div>
            <h3 class="text-xs font-black uppercase tracking-widest mb-1" style="color: rgba(255,255,255,0.8) !important">Kas Organisasi</h3>
            <p class="text-3xl font-black leading-none" style="color: #ffffff !important">{{ formatRupiah(totalBalance) }}</p>
          </div>
          <div class="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-white backdrop-blur-sm border border-white/20 shadow-inner">
            <font-awesome-icon icon="dollar-sign" class="text-xl" />
          </div>
        </div>
        <div class="mt-4 text-[10px] font-black uppercase tracking-wider relative z-10" style="color: rgba(255,255,255,0.9) !important">Saldo saat ini</div>
      </div>
      
      <div class="bg-gradient-to-br from-amber-500 to-orange-500 rounded-lg p-6 shadow-[0_10px_30px_-10px_rgba(245,158,11,0.5)] border border-amber-400/30 relative overflow-hidden">
        <div class="absolute -top-6 -right-6 w-32 h-32 rounded-full bg-white opacity-10 blur-2xl pointer-events-none"></div>
        <div class="flex items-start justify-between relative z-10">
          <div>
            <h3 class="text-xs font-black uppercase tracking-widest mb-1" style="color: rgba(255,255,255,0.8) !important">Agenda & Rapat</h3>
            <p class="text-3xl font-black leading-none" style="color: #ffffff !important">{{ totalAgendas }}</p>
          </div>
          <div class="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-white backdrop-blur-sm border border-white/20 shadow-inner">
            <font-awesome-icon icon="calendar" class="text-xl" />
          </div>
        </div>
        <div class="mt-4 text-[10px] font-black uppercase tracking-wider relative z-10" style="color: rgba(255,255,255,0.9) !important">Telah dipublikasi</div>
      </div>
    </div>

    <!-- Restructured Bottom Layout: Split 2/3 and 1/3 Grid -->
    <div class="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
      
      <!-- Left Column: Unpaid Cash Dues (2 Cols) -->
      <div class="lg:col-span-2 bg-white border border-slate-200/70 rounded-lg overflow-hidden shadow-xs flex flex-col justify-between">
        <div>
          <!-- Header and controls -->
          <div class="p-6 border-b border-slate-150 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-slate-50/50">
            <div>
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto">
              ...
              <div class="relative dropdown-show-container">
                <button 
                  ...
                  class="px-3 py-1.5 bg-white border border-slate-200 hover:border-slate-350 text-slate-700 font-bold text-xs rounded-lg flex items-center gap-2 cursor-pointer transition-all focus:outline-none focus:ring-1 focus:ring-emerald-500 shadow-xs"
                >
                  <span>{{ perPage }}</span>
              ...
              <input v-model="searchQuery" type="text" placeholder="Cari nama..." class="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs text-slate-800 placeholder-slate-450 focus:outline-none focus:ring-1 focus:ring-emerald-500 w-full sm:w-40">
            </div>
          </div>
      ...
      <!-- Right Column: Recent Activity Log (1 Col) -->
      <div class="lg:col-span-1 bg-white border border-slate-200/70 rounded-lg overflow-hidden shadow-xs flex flex-col">
