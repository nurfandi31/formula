...
const activeTab = ref('agenda') // 'agenda' or 'rapat'
...
<template>
  <AdminLayout>
    <!-- 1. Top Card: Modul Selector (Agenda vs Hasil Rapat) - Flush & Merged -->
    <div class="bg-white border border-slate-200/70 rounded-md shadow-xs mb-6 grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-200 overflow-hidden">
      <!-- Pilihan A: Agenda Kegiatan -->
      <button 
        @click="activeTab = 'agenda'" 
        :class="[
          'flex items-center gap-4 p-4 text-left cursor-pointer transition-all duration-350 outline-none border-t-2 border-b-2 border-l-0 border-r-0 md:border-t-2 md:border-b-2 md:border-l-0 md:border-r-0',
          activeTab === 'agenda' 
            ? 'bg-emerald-50/20 border-t-emerald-500 border-b-emerald-500 font-black' 
            : 'bg-white border-t-transparent border-b-transparent hover:bg-slate-50/50'
        ]"
      >
        <div :class="[
          'w-10 h-10 rounded-md flex items-center justify-center flex-shrink-0 border transition-all duration-300',
          activeTab === 'agenda' 
        ...
        <div>
          <div class="flex items-center gap-2">
            <h4 class="text-xs font-black uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
              Agenda Kegiatan
              <span v-if="activeTab === 'agenda'" class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shadow-sm"></span>
            </h4>
            <span v-if="socialStore.agendaKegiatan?.length > 0" :class="[
              'text-[9px] font-black px-2 py-0.5 rounded-full uppercase transition-all duration-300',
              activeTab === 'agenda' ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-650'
      ...
      <!-- Pilihan B: Hasil Keputusan Rapat -->
      <button 
        @click="activeTab = 'rapat'" 
        :class="[
          'flex items-center gap-4 p-4 text-left cursor-pointer transition-all duration-350 outline-none border-t-2 border-b-2 border-l-0 border-r-0 md:border-t-2 md:border-b-2 md:border-l-0 md:border-r-0',
          activeTab === 'rapat' 
            ? 'bg-teal-50/20 border-t-teal-500 border-b-teal-500 font-black' 
            : 'bg-white border-t-transparent border-b-transparent hover:bg-slate-50/50'
        ]"
      >
        <div :class="[
        ...
        <div>
          <div class="flex items-center gap-2">
            <h4 class="text-xs font-black uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
              ...
              <span v-if="activeTab === 'rapat'" class="w-1.5 h-1.5 rounded-full bg-teal-500 animate-pulse shadow-sm"></span>
            </h4>
    ...
    <!-- 2. Dynamic Content: Agenda Section -->
    <div v-if="activeTab === 'agenda'" class="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <!-- Form Input (Kiri: 5 Kolom) -->
      <div class="lg:col-span-5 bg-white border border-slate-200/70 rounded-lg p-6 shadow-xs h-fit">
      ...
      <!-- List Agenda (Kanan: 7 Kolom) -->
      <div class="lg:col-span-7 bg-white border border-slate-200/70 rounded-lg p-6 shadow-xs h-fit">
        ...
        <div class="space-y-1">
          <div v-if="!socialStore.agendaKegiatan || socialStore.agendaKegiatan.length === 0" class="text-center py-8 text-xs text-slate-450 font-bold uppercase tracking-wider">
            Belum ada agenda terpublikasi
          </div>
          <div v-else v-for="agenda in socialStore.agendaKegiatan" :key="agenda.id" class="py-4 flex items-center justify-between gap-4 border-b border-slate-100 last:border-0 hover:bg-slate-50/50 px-2 rounded-lg transition-colors animate-in fade-in duration-200">
            <div class="flex items-center gap-4">
              <div class="w-10 h-10 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center flex-shrink-0 shadow-sm border border-emerald-100">
                <font-awesome-icon icon="calendar" class="text-sm" />
              </div>
              <div>
                <h4 class="text-xs font-black text-slate-855 leading-tight uppercase">{{ agenda.judul }}</h4>
                <div class="flex flex-wrap items-center gap-x-3 gap-y-1.5 mt-2 text-[10px] text-slate-500 font-medium">
                  <span class="bg-slate-100 border border-slate-200 text-slate-650 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider">{{ agenda.kategori || 'Umum' }}</span>
            ...
            <button @click="deleteAgenda(agenda.id)" class="p-2.5 bg-rose-50 border border-rose-100 text-rose-600 hover:bg-rose-600 hover:text-white rounded-md transition-colors cursor-pointer flex-shrink-0 shadow-2xs flex items-center justify-center">
    ...
    <!-- 3. Dynamic Content: Hasil Rapat Section -->
    <div v-if="activeTab === 'rapat'" class="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <!-- Form Input (Kiri: 5 Kolom) -->
      <div class="lg:col-span-5 bg-white border border-slate-200/70 rounded-lg p-6 shadow-xs h-fit">
        <h3 class="text-xs font-black uppercase tracking-widest text-slate-800 mb-6 flex items-center gap-2">
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model.number="rapatPesertaCount" type="number" placeholder="15" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-md text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500">
            </div>
          </div>
          <div>
      ...
      <!-- List Hasil Rapat (Kanan: 7 Kolom) -->
      <div class="lg:col-span-7 bg-white border border-slate-200/70 rounded-lg p-6 shadow-xs h-fit">
        <h3 class="text-xs font-black uppercase tracking-widest text-slate-800 mb-4 flex items-center gap-2">
          <font-awesome-icon icon="file-alt" class="text-teal-500 text-sm" /> Hasil Rapat Terpublikasi
        </h3>
        <div class="space-y-1">
          <div v-if="!socialStore.hasilRapat || socialStore.hasilRapat.length === 0" class="text-center py-8 text-xs text-slate-450 font-bold uppercase tracking-wider">
            Belum ada hasil rapat terpublikasi
          </div>
          <div v-else v-for="rapat in socialStore.hasilRapat" :key="rapat.id" class="py-4 flex items-start justify-between gap-4 border-b border-slate-100 last:border-0 hover:bg-slate-50/50 px-2 rounded-lg transition-colors animate-in fade-in duration-200">
            <div class="flex items-start gap-4">
              <div class="w-10 h-10 rounded-lg bg-teal-50 text-teal-600 flex items-center justify-center flex-shrink-0 shadow-sm border border-teal-100 mt-0.5">
                <font-awesome-icon icon="file-alt" class="text-sm" />
              </div>
              <div class="flex-1 min-w-0">
                <h4 class="text-xs font-black text-slate-855 leading-tight uppercase">{{ rapat.judul }}</h4>
                <div class="flex flex-wrap items-center gap-x-3 gap-y-1.5 mt-2 text-[10px] text-slate-500 font-medium">
            ...
            <button @click="deleteRapat(rapat.id)" class="p-2.5 bg-rose-50 border border-rose-100 text-rose-600 hover:bg-rose-600 hover:text-white rounded-md transition-colors cursor-pointer flex-shrink-0 shadow-2xs flex items-center justify-center mt-0.5">
              <font-awesome-icon icon="trash" class="text-xs" />
            </button>
          </div>
        </div>
