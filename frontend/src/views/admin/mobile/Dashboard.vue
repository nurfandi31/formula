...
<template>
  <div class="space-y-6 px-1">
    <div class="flex flex-col gap-3">
      <div class="bg-gradient-to-br from-indigo-500 to-blue-600 rounded-2xl py-3.5 px-4 shadow-md border border-indigo-400/25 relative overflow-hidden">
        <div class="flex items-center justify-between">
          <div class="space-y-0.5">
            <h3 class="text-[10px] font-black uppercase tracking-wider text-indigo-100/90">Total Pengurus</h3>
            <p class="text-2xl font-black text-white leading-none">{{ totalUsers }} Orang</p>
          </div>
          <div class="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white border border-white/20">
            <font-awesome-icon icon="users" class="text-base" />
          </div>
        </div>
      </div>

      <div @click="goToFinancials" class="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl py-3.5 px-4 shadow-md border border-emerald-400/25 relative overflow-hidden active:scale-[0.98] transition-transform cursor-pointer">
        <div class="flex items-center justify-between">
          <div class="space-y-0.5">
            <h3 class="text-[10px] font-black uppercase tracking-wider text-emerald-100/90">Kas Organisasi</h3>
            <p class="text-2xl font-black text-white leading-none">{{ formatRupiah(totalBalance) }}</p>
          </div>
          <div class="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white border border-white/20">
            <font-awesome-icon icon="dollar-sign" class="text-base" />
          </div>
        </div>
      </div>

      <div class="bg-gradient-to-br from-amber-500 to-orange-500 rounded-2xl py-3.5 px-4 shadow-md border border-amber-400/25 relative overflow-hidden">
        <div class="flex items-center justify-between">
          <div class="space-y-0.5">
            <h3 class="text-[10px] font-black uppercase tracking-wider text-amber-100/90">Agenda & Rapat</h3>
            <p class="text-2xl font-black text-white leading-none">{{ totalAgendas }} Acara</p>
          </div>
          <div class="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white border border-white/20">
            <font-awesome-icon icon="calendar" class="text-base" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white border border-slate-200/80 rounded-2xl p-5 space-y-4 shadow-xs">
      ...
      <div class="overflow-y-auto max-h-[345px] pr-1 py-1 space-y-3 scrollbar-thin">
        <div 
