    ...
    <html>
      <head>
        ...
        <style>
          ...
          body { font-family: 'Inter', sans-serif; color: #1e293b; margin: 0; padding: 8mm; box-sizing: border-box; background: #fff; }
          .receipts-page { display: grid; grid-template-columns: repeat(2, 1fr); grid-template-rows: repeat(3, 1fr); gap: 8mm; width: 100%; height: 275mm; box-sizing: border-box; page-break-inside: avoid; }
          .receipt-card { border: 2px dashed #cbd5e1; border-radius: 12px; padding: 14px 18px; box-sizing: border-box; display: flex; flex-direction: column; justify-content: space-between; height: 100%; background: #fff; }
          .header { text-align: center; border-bottom: 1.5px dashed #cbd5e1; padding-bottom: 6px; margin-bottom: 6px; }
          .logo { font-size: 15px; font-weight: 900; color: #059669; letter-spacing: 0.5px; }
          .subtitle { font-size: 8px; text-transform: uppercase; color: #64748b; font-weight: 700; margin-top: 1px; letter-spacing: 0.3px; }
          .title { font-size: 10px; font-weight: 800; text-transform: uppercase; margin-top: 3px; color: #0f172a; letter-spacing: 0.5px; }
          .body-info { flex-grow: 1; display: flex; flex-direction: column; justify-content: center; gap: 6px; margin: 6px 0; }
          .row { display: flex; align-items: center; font-size: 10px; }
          .label { color: #64748b; font-weight: 600; width: 65px; flex-shrink: 0; }
          .value { font-weight: 700; color: #0f172a; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
          .nominal { font-size: 13px; font-weight: 900; border-top: 1px dashed #f1f5f9; border-bottom: 1px dashed #f1f5f9; padding: 5px 0; text-align: center; margin: 6px 0; letter-spacing: 0.3px; }
          .nominal-pemasukan { color: #059669; background: #ecfdf5; border-color: #a7f3d0; }
          .nominal-pengeluaran { color: #e11d48; background: #fff1f2; border-color: #fecdd3; }
          .footer { text-align: center; font-size: 7px; color: #94a3b8; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-top: auto; }
        </style>
      </head>
      <body>
        ${receiptContent}
...
<template>
  <div>
      ...
      <div class="bg-white border border-slate-200/70 rounded-2xl p-6 col-span-2 shadow-xs">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <textarea v-model="transactionLabel" rows="3" placeholder="Contoh: Konsumsi Rapat Bulanan, pembelian ATK..." class="w-full px-4 py-3 bg-white border border-slate-200 rounded-md text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500 resize-none font-sans"></textarea>
          </div>
    ...
    <div class="mt-8 bg-white border border-slate-200/70 rounded-2xl p-6 shadow-xs print:hidden">
    ...
    <div 
      ...
      class="mt-8 bg-white border border-slate-200/70 rounded-2xl p-5 flex flex-row justify-between items-center gap-4 hover:border-emerald-500/50 hover:shadow-md transition-all cursor-pointer shadow-xs group print:hidden"
    >
      ...
      <button 
        ...
        class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-2 shadow-xs hover:shadow-sm"
      >
    ...
    >
      <div class="bg-white border border-slate-200 rounded-2xl w-full max-w-5xl h-[88vh] max-h-[88vh] flex flex-col shadow-2xl overflow-hidden transform transition-all duration-300 ease-out scale-100">
        ...
        <div class="p-5 border-b border-slate-150 flex flex-row gap-4 justify-between items-center bg-white">
          ...
          <input 
            ...
            class="px-3.5 py-2 bg-white border border-slate-200 rounded-md text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500 w-full sm:w-64"
          >
        </div>
              ...
              <tr v-for="(item, index) in filteredTransactions" :key="index" class="hover:bg-slate-50 transition-colors">
                ...
                <td class="p-4 text-center flex items-center justify-center gap-2">
                  <button 
                    ...
                    class="p-2 bg-emerald-50 border border-emerald-250 rounded-xl text-emerald-700 hover:bg-emerald-600 hover:text-white transition-all cursor-pointer shadow-xs"
                  >
                    <font-awesome-icon icon="print" class="text-xs" />
                  </button>
                  <button 
                    @click="deleteTransaction(index)" 
                    class="p-2 bg-rose-50 border border-rose-250 rounded-xl text-rose-700 hover:bg-rose-600 hover:text-white transition-all cursor-pointer shadow-xs"
                  >
        ...
        <div class="p-5 border-t border-slate-150 bg-slate-50/50 flex flex-row justify-between items-center gap-4 text-[10px] font-black uppercase tracking-wider text-slate-500">
          ...
          <div class="flex items-center gap-2.5">
            ...
            <button 
              @click="closeHistoryModal" 
              class="px-5 py-2.5 bg-slate-200 hover:bg-slate-350 text-slate-700 rounded-xl transition-all cursor-pointer shadow-xs font-bold"
            >
              Tutup
            </button>
          </div>
